...
@pytest.fixture
def mock_decompensation_data(tmp_path):
    """Create mock data with death_hours labels for decompensation sliding window tests."""
    data_dir = tmp_path / "decomp_sw"
    data_dir.mkdir(parents=True)
    ...
    # Stay 1: survivor (inf), Stay 2: death at 80h, Stay 3: death at 30h, Stay 4: survivor
    labels_df = pl.DataFrame(
        {
            "stay_id": [1, 2, 3, 4],
            "decompensation": [float("inf"), 80.0, 30.0, float("inf")],
        }
    )
    labels_df.write_parquet(data_dir / "labels.parquet")
...
class TestSlidingWindowDecompensation:
    """Tests for SlidingWindowDataset with decompensation_pred_hours."""

    def test_decompensation_mode_produces_binary_labels(self, mock_decompensation_data):
        """Labels should be 0.0 or 1.0 in decompensation mode."""
        base = ICUDataset(
            mock_decompensation_data,
        ...
        for i in range(len(sw)):
            label = sw[i]["label"].item()
            assert label in (0.0, 1.0), f"Label should be binary, got {label}"

    def test_observation_death_exclusion(self, mock_decompensation_data):
        """Stay 3 (death at 30h): windows where 30 is in [t, t+48) excluded."""
        base = ICUDataset(
            mock_decompensation_data,
            task_name="decompensation",
            normalize=False,
        ...
        for i in range(len(sw)):
            ...
            if sample["stay_id"] == 3:
                ...
                assert not (
                    t_start <= 30 < obs_end
                ), f"Stay 3 window at t={t_start} should be excluded (death at 30)"

    def test_without_decompensation_passes_stay_labels(self, mock_decompensation_data):
        """Without decompensation_pred_hours, labels are passed through unchanged."""
        base = ICUDataset(
            mock_decompensation_data,
            task_name="decompensation",
            normalize=False,
        ...
        sw = SlidingWindowDataset(
            ...
            window_size=48,
            stride=24,
            # No decompensation_pred_hours
        )

        # Labels should be the raw death_hours values
        for i in range(len(sw)):
            sample = sw[i]
            label = sample["label"].item()
            # Should be death_hours value (inf or some float), not binary
            assert label >= 0.0  # Either 0, positive, or inf
