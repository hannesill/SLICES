...
It intentionally focuses on implementation status and data semantics rather than
freezing a set of thesis numbers that may change as revisions are rerun. The
canonical outputs are the `fairness/*` keys written to W&B summaries and the
parquet exports produced by `scripts/export_results.py`.
...
- Default thesis fairness corpus:
  - Sprints `1`, `2`, `3`, `4`, `5`, `7p`, `10`, `11`, `12`, `13`
...
- eICU race is excluded from the thesis fairness sweep.
...
1. Threshold-dependent classification fairness metrics use a fixed threshold of
   `0.5`.
2. Race fairness follows the thesis plan's canonical five-bin schema rather than
   raw source strings.
3. Subgroup inclusion is patient-based, with the default threshold set to `50`
   patients.
4. The standalone default targets the full thesis downstream corpus, including
   the canonical Sprint `11` classical baselines.
...
This file should not be used as the frozen numeric appendix for the thesis.
